...
├── LICENSE                           # MIT
├── README.md
├── .connector-sync.json              # Which connector commit the tool tables were generated from
├── scripts/
│   ├── sync-from-connector.py        # Regenerates the tool tables from the connector's tool-inventory.json
│   ├── skill-map.json                # Which connector tool group / tool lands in which skill section
│   └── validate.py                   # Structural checks run in CI on every PR
├── .github/workflows/validate.yaml   # CI: runs scripts/validate.py
...
## How the Skills Stay in Sync with the Connector

The connector is the source of truth. Every tool table in this repository, in the six skills and in this README, sits between `BEGIN GENERATED` / `END GENERATED` markers and is produced by `scripts/sync-from-connector.py` from the connector's `docs/tool-inventory.json` (itself generated from the tool registry by `make tool-inventory`, with a CI test that fails when it is stale). Tool names, groups, one-line descriptions, write / destructive markers and feature-flag notes all come from that file. `.connector-sync.json` records which connector commit the tables were generated from.

When the connector's tool surface or doctrine changes on `main`, its `sync-kiro-power` workflow runs this script and opens a pull request here. The PR lists added and removed tools and the connector doctrine files (skill pack, prompts, terminology, CNAPP sources) that changed since the last sync, so the reviewer folds prose changes into the affected skill by hand. A brand-new tool lands in a visible "needs placement" section until someone assigns it to a section in `scripts/skill-map.json` and writes its workflow; `scripts/validate.py` keeps that PR red until then.

To run it locally against a connector checkout:

```bash
python3 scripts/sync-from-connector.py --connector ../native-mcp-connector --write
python3 scripts/validate.py
```

Edit prose anywhere outside the generated blocks. Never edit inside them; the next sync overwrites it.
